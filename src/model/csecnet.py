...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.conv2(x)
        x += residual  # Add the original input (skip connection)
        return x
...
class LitModel(SingleNetBaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_lambda_map = {
            # Vinh: Add this line for MSE Loss ---
            'MSE_LOSS': lambda: F.mse_loss(output_batch, gt_batch),            
...
class DeepWBNet(nn.Module):
    ...
    def __init__(self, opt=None):
        ...
        self.out_net = nn.Sequential(
            nn.Conv2d(9, nf, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            ResidualBlock(nf),  # Vinh: First residual block
            ResidualBlock(nf),  # Vinh: Second residual block
            NONLocalBlock2D(nf, sub_sample="bilinear", bn_layer=False),  # Non-local attention
            nn.Conv2d(nf, nf, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf, 3, kernel_size=1),
            NONLocalBlock2D(3, sub_sample="bilinear", bn_layer=False)
        )
